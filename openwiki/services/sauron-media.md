...
qBittorrent is enabled with group `media`, profile `/var/lib/qbittorrent`, web UI port 8088, and `/media/torrent/download` plus `.incomplete` paths. `tmpfiles` creates `/media` as `root:root 0755`; `/media/usenet`, `incomplete`, and `complete` as setgid `2775` paths shared by `sabnzbd:media`, with a recursive `Z` permission repair on completed content; `/media/jelly`, `shows`, and `movies` are setgid `2775` with Jellyfin as owner for the library directories. SABnzbd has group `media` and `UMask = "0002"`; Sonarr and Radarr also use group `media`; Jellyfin reads `/media/jelly`. The critical invariant is group readability across downloader, organizer, and Jellyfin: do not alter paths, service/group ownership, tmpfiles modes, or UMask in isolation.
